...
/// invalidation. Deterministic across runs (unlike `DefaultHasher`).
...
/// Load a cached skill embedding from `skill_dir` if it matches `content`.
///
/// Cache layout (both files live inside the skill's own subdirectory):
/// - `SKILL.embedding`      — raw `f32` values, little-endian
/// - `SKILL.embedding.hash` — FNV-1a hex of the skill content used when the
///                            embedding was computed; acts as a staleness check
///
/// Returns `None` when the cache is missing, unreadable, or stale.
async fn load_cached_skill_embedding(skill_dir: &Path, content: &str) -> Option<Vec<f32>> {
...
/// Persist `embedding` alongside the skill file so future loads skip the API call.
async fn save_skill_embedding_cache(skill_dir: &Path, content: &str, embedding: &[f32]) {
...
///
/// ## Scoring
/// When `query_embedding` is provided, each skill is scored by cosine similarity
/// against a cached embedding stored alongside the skill file. If no cached
/// embedding exists, one is computed via `embedding_service` and written to disk
/// (`SKILL.embedding` + `SKILL.embedding.hash`) so subsequent calls are free.
/// When neither a cached embedding nor an embedding service is available, scoring
/// falls back to keyword overlap (fraction of query words found in name + content).
///
/// ## Uses `tokio::fs` throughout to avoid blocking Tokio worker threads.
pub async fn load_local_skill_hits(
    skills_dir: &Path,
    query: &str,
    ...
    while let Ok(Some(entry)) = dir.next_entry().await {
        ...
        let score = if let Some(query_emb) = query_embedding {
            // Resolve skill embedding: cached → computed+cached → keyword fallback
            let skill_emb = match load_cached_skill_embedding(&path, &content).await {
                Some(emb) => Some(emb),
                None => match embedding_service {
                    Some(svc) => match svc.embed(&content).await {
                        Ok(emb) => {
                            save_skill_embedding_cache(&path, &content, &emb).await;
                            Some(emb)
                        }
                        Err(e) => {
                            warn!(
