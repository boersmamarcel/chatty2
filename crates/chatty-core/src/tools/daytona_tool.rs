...
impl DaytonaTool {
    ...
    /// Get the sandbox working/project directory.
    async fn get_project_dir(&self, sandbox_id: &str) -> Option<String> {
        let url = format!(
    ...
    ///
    /// Uses two strategies:
    /// 1. Parse the code for explicit output paths (savefig, write_html, etc.)
    /// 2. Scan common directories for files with downloadable extensions
    async fn discover_files(&self, sandbox_id: &str, code: &str) -> Vec<String> {
        let mut found = Vec::new();

        // Strategy 1: Try explicit paths extracted from the code
        let explicit_paths = extract_output_paths(code);
        let project_dir = self.get_project_dir(sandbox_id).await;

        if !explicit_paths.is_empty() {
            info!(sandbox_id, paths = ?explicit_paths, project_dir = ?project_dir, "Checking explicit output paths from code");
        }
        ...
        for path in &explicit_paths {
            ...
            let candidates = if path.starts_with('/') {
                vec![path.clone()]
            } else if let Some(ref pdir) = project_dir {
                vec![
                    format!("{}/{}", pdir.trim_end_matches('/'), path),
                    format!("/tmp/{}", path),
                ]
            } else {
                vec![
        ...
        // Strategy 2: Scan common directories for any downloadable files we missed
        let mut scan_dirs: Vec<&str> = vec!["/tmp"];
        let project_dir_str;
    ...
    async fn download_generated_files(&self, sandbox_id: &str, code: &str) -> Vec<String> {
        let remote_files = self.discover_files(sandbox_id, code).await;
...
impl Tool for DaytonaTool {
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        let downloaded_files = if exit_code == 0 {
            self.download_generated_files(&sandbox_id, &code).await
        } else {
            Vec::new()
        };
