...
static USER_SECRETS_REPOSITORY: OnceLock<Arc<dyn settings::repositories::UserSecretsRepository>> =
    OnceLock::new();
static BROWSER_CREDENTIALS_REPOSITORY: OnceLock<
    Arc<dyn settings::repositories::BrowserCredentialsRepository>,
> = OnceLock::new();
...
pub fn init_repositories() -> anyhow::Result<()> {
    ...
    USER_SECRETS_REPOSITORY
        ...
        .ok();
    BROWSER_CREDENTIALS_REPOSITORY
        .set(Arc::new(BrowserCredentialsJsonRepository::new()?))
        .ok();
...
/// Returns a cloned Arc to the browser credentials repository.
pub fn browser_credentials_repository()
-> Arc<dyn settings::repositories::BrowserCredentialsRepository> {
    BROWSER_CREDENTIALS_REPOSITORY
        .get()
        .expect("init_repositories() not called")
