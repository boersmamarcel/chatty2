...
static USER_SECRETS_REPOSITORY: OnceLock<Arc<dyn settings::repositories::UserSecretsRepository>> =
    OnceLock::new();
static BROWSER_SETTINGS_REPOSITORY: OnceLock<
    Arc<dyn settings::repositories::BrowserSettingsRepository>,
> = OnceLock::new();
...
pub fn init_repositories() -> anyhow::Result<()> {
    ...
    BROWSER_SETTINGS_REPOSITORY
        .set(Arc::new(BrowserSettingsJsonRepository::new()?))
        .ok();
...
/// Returns a cloned Arc to the browser settings repository.
pub fn browser_settings_repository() -> Arc<dyn settings::repositories::BrowserSettingsRepository> {
    BROWSER_SETTINGS_REPOSITORY
        .get()
        .expect("init_repositories() not called")
        .clone()
