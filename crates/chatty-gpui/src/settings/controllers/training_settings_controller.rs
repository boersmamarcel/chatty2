...
/// Toggle JSONL auto-export enabled/disabled and persist to disk
pub fn toggle_jsonl_auto_export(cx: &mut App) {
    // 1. Apply update immediately (optimistic update)
    let old_enabled = cx.global::<TrainingSettingsModel>().jsonl_auto_export;
    let new_enabled = !old_enabled;
    info!(
        old = old_enabled,
        new = new_enabled,
        "Toggling JSONL auto-export"
    );
    cx.global_mut::<TrainingSettingsModel>().jsonl_auto_export = new_enabled;
