    ...
    P: PromptHook<M>,
{
    ...
    ///
    /// This will cause the agent to execute tools concurrently when the LLM
    /// emits multiple tool calls in a single assistant message.
    ...
    async fn send(self) -> StreamingResult<M::StreamingResponse> {
        ...
        let stream = async_stream::stream! {
            ...
            'outer: loop {
                ...
                // When concurrency > 1, tool calls are collected here and executed
                // after the inner stream ends, using buffer_unordered for parallelism.
                let mut pending_concurrent_tool_calls: Vec<(crate::message::ToolCall, String)> = vec![];
                ...
                while let Some(content) = stream.next().await {
                    match content {
                        ...
                        Ok(StreamedAssistantContent::ToolCall { tool_call, internal_call_id }) => {
                            // Always yield the tool call event immediately so the UI can show "tool started"
                            ...
                            if concurrency > 1 {
                                // Defer execution: collect for concurrent execution after stream ends
                                pending_concurrent_tool_calls.push((tool_call, internal_call_id));
                                saw_tool_call_this_turn = true;
                            } else {
                                // Sequential execution (default): execute inline as before
                ...
                // Execute deferred tool calls concurrently (when concurrency > 1)
                if !pending_concurrent_tool_calls.is_empty() {
                    use futures::stream::StreamExt as FuturesStreamExt;

                    let results: Vec<Result<(crate::message::ToolCall, String, String), _>> =
                        futures::stream::iter(pending_concurrent_tool_calls.drain(..))
                            .map(|(tool_call, internal_call_id)| {
                                let tool_server_handle = tool_server_handle.clone();
                                let hook = self.hook.clone();
                                let chat_history_snap = chat_history.clone();
                                let tool_span = info_span!(
                                    parent: tracing::Span::current(),
                                    "execute_tool",
                                    gen_ai.operation.name = "execute_tool",
                                ...
                                async move {
                                    ...
                                    if let Some(ref hook) = hook {
                                        ...
                                        if let ToolCallHookAction::Terminate { reason } = action {
                                            return Err(cancelled_prompt_error(&chat_history_snap, reason).await);
                                        }
                                        ...
                                        if let ToolCallHookAction::Skip { reason } = action {
                                            ...
                                            return Ok((tool_call, internal_call_id, reason));
                                        }
                                    }
                                    ...
                                    Ok((tool_call, internal_call_id, tool_result))
                    ...
                    for result in results {
                        match result {
                            Ok((tool_call, internal_call_id, result_text)) => {
                                let tool_call_msg = AssistantContent::ToolCall(tool_call.clone());
                                tool_calls.push(tool_call_msg);
                                tool_results.push((tool_call.id.clone(), tool_call.call_id.clone(), result_text.clone()));
                                let tr = ToolResult {
                                    id: tool_call.id,
                                ...
                                yield Ok(MultiTurnStreamItem::StreamUserItem(
                                    StreamedUserContent::ToolResult { tool_result: tr, internal_call_id }
                                ));
                            }
                            Err(e) => {
                                yield Err(e);
