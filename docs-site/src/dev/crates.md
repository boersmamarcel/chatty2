...
**When to read this:** You need a one-glance map of the 13 workspace crates and where to dive deeper. This page opens the Reference section.

The Reference section is lookup material, most of it generated or synced from the repository:
the crate index below (one README per crate), the [tools catalog](./reference/tools-catalog.md), the [provider matrix](./reference/provider-matrix.md), the [slash commands](./reference/slash-commands.md), the [CLI flags](./reference/cli-flags.md), the [environment variables](./reference/env-vars.md), the [settings schema](./reference/settings-schema.md), the [GPUI event catalog](./reference/event-catalog.md), the [singleton inventory](./reference/singleton-inventory.md),
the [WIT interface](./architecture/wit-reference.md), the [curated MCP catalog](./architecture/curated-mcp-catalog.md), [build disk usage](./architecture/build-disk-usage.md),
[make targets & CI workflows](./ci-reference.md), the [doc frontmatter schema](./doc-frontmatter.md) and the [glossary](./glossary.md).
The `reference/*` pages are regenerated by `make docs-gen` from `scripts/gen-docs-reference.sh`; edit the script, not the page.
