...
- **The stall watchdog** wakes every `STALL_TICK` (5 s). After `STALL_TIMEOUT` (180 s) with no chunk and no sub-agent progress it logs `Stream produced nothing for too long; ending the turn as stalled` at `warn` and ends the turn with the error text `The model stopped responding (no output for 3 minutes)…` (the duration is the timeout that fired: while a tool call runs it is `TOOL_STALL_TIMEOUT`, 13 minutes). A long tool call is silence from the stream's point of view, which is why the timeout is generous. `--headless` resumes a stalled turn itself — a short "continue" prompt on the same history, at most `HEADLESS_STALL_RESUME_ATTEMPTS` (2) times per task, logged as `Auto-resuming the stalled turn (n/2).` on stderr; the desktop and interactive TUI leave that to the human.
