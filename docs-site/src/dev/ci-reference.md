...
| **Reserved symbols** (`reserved.yml`) | PR / push to `main` / merge group | Required check: fails if a symbol listed in `RESERVED.md` is implemented without its `todo!("human: …")` marker or a `// HUMAN-WRITTEN:` attestation (`scripts/check-reserved.sh`). |
| **Auto-arm** (`auto-arm.yml`) | PR opened / reopened / marked ready for review | Arms GitHub auto-merge (squash) on same-repo PRs from the repository owner, `github-actions[bot]` or a docs bot's `docs/*` branch, so every ordinary PR lands by itself once required checks are green and the branch is current. Hold a PR back by opening it as a draft. Uses `AUTO_UPDATE_TOKEN` so the merge still triggers the push-to-`main` workflows. |
| **Update next queued PR** (`update-next-queued-pr.yml`) | Push to `main`, auto-merge enabled on a PR, every 30 min, or manual | The one-lane queue: presses "update branch" on the oldest auto-merge-armed PR so PRs land one at a time under the branch-must-be-current ruleset. |
| **PR nurse** (`pr-nurse.yml`) | Hourly, after any CI / Reserved / Ship-auto guard run completes, or manual | Picks one auto-merge-armed PR that is conflicted or has a red required check and has a Sonnet session fix it (`scripts/pr-nurse-triage.sh`), pushing a `nurse:` commit; after two attempts it disarms auto-merge and labels `blocked:human`. Never merges. |
| **Ship-auto guard** (`ship-auto-guard.yml`) | PR opened / synchronized / (un)labeled, merge group | Required check that no-ops on ordinary PRs; on a `release:*`-labeled PR enforces the path deny-list, and for `ship:auto` additionally patch-only and an `auto/*` branch. |
| **Ship-auto merge** (`ship-auto-merge.yml`) | PR opened / reopened / labeled / synchronized / ready for review | Enables squash auto-merge for `ship:auto` PRs from same-repo heads when the sender is the owner or `github-actions[bot]` — the zero-human patch-release path. |
| **Privileged labels** (`privileged-labels.yml`) | PR labeled | Only the repository owner and `github-actions[bot]` may apply `ship:auto`, `release:patch|minor|major` or `cut-release`; anyone else has the label removed and gets a comment. |
