...
**Page extracts in search results.** The top three search results also carry a short extract of the page itself: the passages that match the query, or the page's opening when none do (up to about 1,200 characters each and 4,000 per search). The agent can often read the fact it needs straight from the results instead of fetching each page, which saves turns and context. The pages are read the same way fetch reads them, with the same address filtering, and are kept in the session's page cache, so a follow-up fetch of the same page does not download it again. A page that is slow or fails just has no extract; the search itself never waits more than a few seconds for them. Turn it off with **Page Extracts** under **Settings → Internet → Web Search**.
