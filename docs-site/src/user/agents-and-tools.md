...
The browser tools drive a real Chrome on your machine, so the agent can look at what it just built instead of guessing: it renders the page, screenshots it, spots the problem, fixes it, and re-checks. It can also click — a button, a link, a menu item — by picking an element out of its own snapshot of the page rather than guessing coordinates; the click is refused if the element moved, is covered by something else, or is off screen. While it works, a live view of the page docks beside the chat; you can click and type in it, and take control at any moment. **Hand back & continue** gives the browser back to the agent and tells it to take a fresh look at the page and carry on from there.
