...
**Human path only.** Do not use this skill for Linear **Chatty auto-ship** /
**Chatty tech debt** / `ship:auto` work — that path labels the PR `ship:auto` +
`release:patch` and relies on auto-merge + `prepare-release`. This skill is for
Marcel-driven releases (PR label or `workflow_dispatch`).
