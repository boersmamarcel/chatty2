...
- **Mermaid Diagrams in the Artifact Panel (AGE-566)**: A markdown file's Rendered tab draws mermaid diagrams instead of leaving them as unrendered code fences. `chatty-gpui/src/chatty/views/transcript/markdown_mermaid.rs`'s `split_mermaid` is a pure, fence-aware splitter (no gpui, no renderer) that walks a document's top-level fences and returns `DocPart::Markdown`/`DocPart::Mermaid` runs in order — a mermaid fence nested inside another code block stays part of that block, and an unclosed one is left as markdown rather than swallowing the rest of the document. `ArtifactView::sync_rendered_doc` re-splits and re-renders (via `MermaidRendererService::render_to_svg_file`) only when the text or the light/dark theme changes (keyed on a content hash + `is_dark`, cached as `RenderedDoc`), not every frame; each `RenderedPart::Mermaid` becomes a `MermaidComponent` (falling back to showing the raw source when rendering fails) and each `RenderedPart::Markdown` run its own `TextView`, joined in document order by `artifact_rendered_markdown`. Since `mermaid-rs-renderer` 0.3 (bumped from 0.2, AGE-572), invalid mermaid syntax is a render error rather than best-effort SVG output — this is what a diagram falling back to source text means in practice — and `MermaidRendererService::CACHE_VERSION` was bumped alongside it to invalidate stale on-disk SVGs.
...
- **Transcript Rendering**: The desktop transcript renders conversation history as typed blocks (`crates/chatty-gpui/src/chatty/views/transcript/`) — turns, tool rows, diffs, plans, artifact cards, approvals, etc. — built from `MessageEntry` + `system_trace` JSON via `adapt_message()`/`adapt_messages()`. Persistence stays untyped in chatty-core; these typed block types live only in chatty-gpui. The list itself renders on gpui's `list`/`ListState`, not gpui-component's `v_virtual_list`: `List` measures each item as it lays it out and caches the result, so turn heights are an output, not a hand-estimated input (the old `TranscriptLayout` estimator was deleted along with every height constant it needed). Use `ListAlignment::Top`, not `Bottom` — `Bottom` re-nulls the scroll anchor every frame while pinned, so `bounds_for_item` returns `None` for items above it; sticky-to-bottom is instead one line (anchor past the last item, let `layout_items` backfill). `set_scroll_handler`'s callback runs while `ListState`'s internal `RefCell` is mutably borrowed, so it may only set a flag — calling back into the list from inside it panics. `adapt_message()` emits a `Block::Plan` per message that called `write_todos`, but every plan block renders the same live conversation-level snapshot, so a follow-up turn that re-plans would otherwise paint the identical panel twice; `retain_last_plan_block()` (`transcript/adapter.rs`) keeps only the newest plan block per adapted turn list. `turn_fingerprint()` (`chat_view/mod.rs`) is what tells `ListState` a turn's rendered height may have changed, so its contract is that a turn whose height *can* change must hash differently — that includes out-of-band state a block itself doesn't carry: `Block::Plan` renders the live `AgentTaskSnapshot`, not its own fields, so the fingerprint takes the snapshot as a separate argument and hashes `write_todos_called`, todo count/status/text-length per todo; `Block::Approval` hashes `approval.command.len()` since the alert card grows with the command (a heredoc runs to many lines) (AGE-338). `ChatView` does not rebuild the typed transcript from scratch on every `cx.notify()` (a streaming turn notifies once per 20ms text batch): `chat_view/turn_cache.rs`'s `TurnCache` keeps last frame's adapted `Turn`s and re-adapts only the messages whose `adapt_key()` moved (index, role, content length, collapse state, shape of the live/persisted trace), so a frame costs O(changed turns) rather than O(history) (AGE-165). Plan-block placement is expressed as ownership rather than the old two destructive passes over a fresh vec: `plan_owner()` picks which turn draws the panel (the newest turn that planned for itself, else the last assistant turn while a snapshot is live) and `apply_plan_ownership()` re-asserts that against whatever the cache holds every frame, so a cached turn converges on what a full rebuild would produce; `retain_last_plan_block()`/`attach_plan_block()` (`transcript/adapter.rs`) still exist and back that logic but are no longer called directly from `ChatView`. The compact plan strip that floats above the transcript once the inline card scrolls out of view is judged on the card's own painted bounds, not its turn's (AGE-559): a `canvas` probe painted over the plan element records its bounds each frame (`ChatView::plan_card_bounds`), and `compute_plan_above_viewport` compares that against the viewport top, falling back to the previous frame's answer when the card didn't paint (outside the rendered range) rather than guessing from row indexes — the turn goes on to hold the whole answer, so judging by the turn's bounds left the strip showing late or never while a turn streamed. The strip itself renders as an absolutely-positioned overlay with a 160ms fade-in (`PLAN_STRIP_FADE_MS`), not a row in document flow, so it never shifts the transcript underneath. `reset_transcript_list()` clears the cache on conversation switch, since cache entries are keyed by message index. While a turn streams, the open line (everything after the last `\n` of the growing text) renders as a `CachedMarkdownSegment::PlainTail` — plain text, never reaching the math parser, markdown parser or Typst — while the settled prefix through the last newline keeps its full parse, reused by `Arc` (`message_parsing.rs`'s `LiveTailCache`) while byte-identical; a newline promotes the line, and stream end promotes it through the unchanged finalized path (AGE-167). Sticky scroll is re-asserted only once per drawn frame, from `prepare_render` (and on activation, from `activate_sticky_scroll`) — never from a stream-event handler like `append_assistant_text`, since those fire once per 20ms text batch regardless of whether a frame is drawn; `chat_view::sticky_scroll_tests` pins the call count at exactly those two sites. `consolidate_receipt_artifacts()` (`transcript/adapter.rs`) also dedups standalone artifact cards (PDF/image/pptx/tabular) that share the same tool-reported path (AGE-488): `dedup_standalone_artifacts` keeps only the *last* occurrence of a path, left at its own trace position rather than moved to the first occurrence's slot (last-write-wins). The dedup key is `Block::Artifact.path` exactly as the tool reported it, not resolved against the workspace, so two tool calls naming the same file with different spellings (an absolute `saved_path` vs. a later relative name) still render as two cards — closing that needs `resolve_artifact_path`'s workspace-aware resolution (AGE-487), deliberately not done here.
- **`terminal_read`: the agent reads the human's tmux panes (AGE-577, stage 0 of the embedded-terminal epic) and, on the desktop, terminals shared from the embedded dock (AGE-583, stage 1's T5, below)**: `chatty_core::services::terminal` (`services/terminal/mod.rs`) defines an async, object-safe `TerminalSource` trait — `list()` returns `TerminalInfo { id, title, cwd?, backend, kind, access }` most-recently-active first, `read(id, Region)` returns `TerminalText { text, cursor_row, cols, rows }` with trailing blanks trimmed — with `Region`/`TerminalBackend`/`TerminalKind` marked `#[non_exhaustive]` so a `LastCommand` region and further kinds can be added without breaking callers. `TerminalKind` is `Human` (a tmux pane, or a tab the human opened) or `Agent` (the dock's future Agent tab, AGE-586, always readable and not the human's to unshare); `TerminalAccess` (`None`/`Read`/`ReadRun`, `can_read()`) is what the human has shared — a tmux pane is shared `Read` as a whole by `terminal_access`, an embedded tab one by one. `TmuxSource` (`services/terminal/tmux.rs`) is the tmux backend: one `list-panes -a -F …` call for listing, one `display-message` (cursor/size) plus `capture-pane -p -J [-S -N]` call for reading, every invocation argv-only (no shell string) with a 5s timeout and `kill_on_drop`; only `%N` pane ids are accepted and `lines` is clamped; no tmux or no server gives an empty list rather than an error, and chatty's own pane (`$TMUX_PANE`) is left out so the default is the pane the user was just in. `TerminalSources` (same module) lists and reads several sources as one — `list` concatenates in the order given (embedded tabs first, so the last-focused *shared* tab beats any tmux pane as the default target) and `read` routes to whichever source's list carries the id. The `terminal_read` tool (args `{ terminal?, lines?, list? }`, output capped at the shell's output budget keeping the tail with a `truncated` note) refuses a named terminal whose `access` is `None` with a message naming it and pointing at the eye icon, and picks the first *readable* terminal (not just the first) as the default target; with nothing shared it says so, distinguishing "no terminals are shared with you" from "N open but none shared". It is gated by `ExecutionSettingsModel::terminal_access` — off by default and left out of the settings file while off, so existing files and the canonical settings-snapshot bytes (AGE-283) are unchanged — and combines the tmux source in only when that setting is on *and* tmux actually has panes at agent-build time; it is excluded from the `coder`/`reviewer`/`coordinator` worker tool profiles, so a hive/Firecracker worker (no tmux, no embedded dock) never gets it. On the desktop, `AgentBuildContext::embedded_terminals` (set only by chatty-gpui, via `embedded_terminal_source`) means the tool is *always* offered regardless of `terminal_access`, since a tab is shared long after the agent is built and access is checked per tab at call time, not at build time; chatty-tui and hive keep the tmux-only gate. Desktop exposes an "Enable Terminal Access" switch under "Enable Code Execution" (for tmux) and the transcript verb "Read terminal · `<tab>` · N lines" once a terminal was actually read (`transcript/verb.rs`'s `terminal_read_subject`, grouped as a tool, not folded into "Explored N files"); chatty-tui adds `terminal` as a `--enable`/`--disable`/`--only`/`/tools` group name (see the tool-group list under Unattended-run robustness below) with the same verb, tmux-only. Every embedded read also checks `TerminalHandle::input_hidden()` (below) and refuses with `terminal::HIDDEN_INPUT_MESSAGE` at a password prompt instead of returning the screen.
- **Embedded terminal, gpui element (AGE-579, stage 1 of the embedded-terminal epic: T1 element → T2 input (AGE-581) → T3 dock (AGE-582) → T5 sharing with the agent (AGE-583, below) → T4 agent shell tab (AGE-586))**: `crates/chatty-gpui/src/chatty/views/terminal/` (`TerminalView`) owns a `chatty-terminal` `TerminalHandle` (now behind an `Arc`, so the sharing registry below can hold a `Weak` to it; PTY + `alacritty_terminal::Term`, no UI of its own) and paints its grid with one custom `Element` (`element.rs`'s `TerminalElement`) rather than a div per cell. `grid.rs` is the toolkit-free half — colour resolution (16 ANSI + 256-cube/grey-ramp + truecolour + OSC overrides, theme-derived light/dark palette via `Palette::from_theme`) and `batch_row()`, which turns a row of cells into a few text runs and merged background rects (a run only splits on a style change; a blank inside a run never splits it; wide/combining characters get their own run) — so it's unit-tested without a window. `element.rs` shapes each row's runs once and caches them by run content (`RowCache`, two-frame LRU-ish retention via `current`/`previous`), and caches the whole painted `Frame` keyed on the terminal's generation counter, size, font and palette: unchanged on all four, the previous frame is replayed with no terminal lock taken at all. Font metrics (`Metrics`) are resolved once per settings change, not per frame — the AGE-378 trap of a missing family/face otherwise being re-resolved (and its failure re-paid) every frame. Repaints are driven by the terminal, not polled: a blocking thread forwards `TerminalHandle`'s std-channel events into gpui, coalescing any events queued behind a `Wakeup` into one `cx.notify()`; an idle terminal paints nothing. Resize follows the element's bounds — the first size applies at once, later ones after `RESIZE_DEBOUNCE` (100ms) of stable size, so dragging a panel edge doesn't reflow the shell every frame. T2 (AGE-581) added keyboard, paste, selection, scrollback and mouse input. T3 (AGE-582) replaced the `CHATTY_DEBUG_TERMINAL=1` debug hook with a real bottom dock under the chat column, VS Code/Zed-style (`crates/chatty-gpui/src/chatty/views/terminal/dock.rs`'s `TerminalDock`): a tab strip (title `program — dir`, from `TerminalHandle::foreground_process_name()`/`current_dir()` on Linux polled every second only while the dock is open, else the shell name and starting directory; full OSC title as the tooltip), a drag handle on the top edge (`MIN_CHAT_HEIGHT` = 160px, `MIN_DOCK_HEIGHT` = 100px), and a maximise toggle that hides the transcript but keeps the composer above the dock. Ctrl/Cmd+J and `` Ctrl+` `` (`TOGGLE_DOCK_KEYS`) open the dock focused or hide it and refocus the composer; `` Ctrl+Shift+` `` (`NEW_TERMINAL_KEYS`, X11's `ctrl-~` bound too) opens a new tab — all live in `keys::RESERVED_KEYS` so they reach the app from inside a focused terminal, with `actions.rs`'s `ToggleTerminalDock`/`NewTerminal` handlers on the app root (`app_view.rs`) since they move `Window` focus. The dock belongs to the window, not a conversation, so switching conversations neither hides nor kills its terminals; closing a tab asks for confirmation only when a foreground job other than the shell is running (`TerminalHandle::has_foreground_job`, via `tcgetpgrp` against a duplicated PTY master fd — always asks on Windows, where that can't be told), closing the last tab hides the dock, and app quit/window close kill every open shell (`TerminalDock::new`'s `cx.on_app_quit` clears `tabs`, dropping each `TerminalHandle`). `GeneralSettingsModel::terminal: TerminalSettings` (`#[serde(default)]`, so an older settings file loads with defaults) holds font family/size (applied live to open terminals), a shell override and `scrollback_lines` (`TerminalConfig::scrollback`, alacritty's own default of 10,000 — both apply only to terminals opened after the change) and `dock_height` (updated by dragging, saved after a 400ms debounce). Keys reach the shell through a per-view keystroke interceptor (`input::intercept_keys`, `cx.intercept_keystrokes`) that runs before gpui matches keybindings and acts only while the terminal's `FocusHandle` is focused: only `keys::RESERVED_KEYS` (Linux/Windows: `ctrl-j`, `` ctrl-` ``, `` ctrl-shift-` ``, `ctrl-shift-c`, `ctrl-shift-v`, `ctrl-q`; macOS: `cmd-j`, `` ctrl-` ``, `` ctrl-shift-` ``, `cmd-c`, `cmd-v`, `cmd-q`) pass through to the app, encodable keys go to the PTY via `keys::encode_key` (cursor keys in normal/CSI vs. application/SS3 mode with xterm modifier parameters, F1–F12, Insert/Delete/PgUp/PgDn, Shift-Tab, CR/CRLF, Backspace, C0 codes for Ctrl+letter, ESC prefix for Alt), and any other Ctrl/Alt chord is swallowed rather than falling through to an app shortcut; printable text still rides gpui's `EntityInputHandler` text-input path so IME commits and dead keys arrive whole. `Copy`/`Paste` are real gpui actions bound to `keys::COPY_KEY`/`PASTE_KEY` under the `Terminal` key context (`terminal::init(cx)`, called from `actions::register_actions`) — paste is bracketed when the program enables it, with the pasted text stripped of ESC and line breaks turned to CR. Selection (drag, double-click word, triple-click line) reuses alacritty's own `Selection`; Shift forces local selection even when the program wants mouse-report mode; `element.rs` paints it as extra background rects in the theme's selection colour. The wheel scrolls scrollback in the normal screen, sends cursor keys in the alternate screen (alternate scroll, for `less`), and is forwarded as a mouse report in mouse mode (SGR 1006, classic `CSI M`, and UTF-8 1005, gated by `keys::wants_mouse` on what the program asked for); any keyboard input snaps the view back to the live screen (`TerminalView::send_input`). OSC 52 clipboard writes are forwarded to the OS clipboard (`TerminalEvent::ClipboardStore`); reads are never answered — `chatty-terminal`'s `Term` is built with `Osc52::OnlyCopy` so the read request is dropped inside `Term` itself. `TerminalHandle::with_term_mut` is the one way to mutate the grid under its lock (scrolling, selection) and bumps `generation` so the view repaints. Known limits: application keypad mode isn't honoured (gpui reports keypad keys as main-row keys), IME pre-edit text isn't drawn, and there's no drag auto-scroll or middle-click primary-selection paste. T5 (AGE-583) shares dock tabs with the agent, per tab. `views/terminal/registry.rs`'s `EmbeddedTerminals` is a process-wide `TerminalSource` (a gpui global via `EmbeddedTerminals::global`, cloned as an `Arc`) that the dock registers every tab into (`register`, on open) and removes from (`remove`, on close or app quit) — an entry holds only a `Weak<TerminalHandle>`, so `terminal_read` lists and reads on its own thread, under the handle's own lock, with no gpui entity or main-thread hop in the path; `list` upgrades every entry under the registry's lock (dropping any whose tab has actually closed) and sorts most-recently-*focused* first (`focused`, called from `TerminalDock::focus_active`), which is what makes the default read target "the shared tab the user was in last" rather than just the newest tab. A human tab registers at `TerminalAccess::None` (listed — id, title, cwd — but never read) until the eye icon on it is clicked: `TerminalDock::share_clicked` unshares a shared tab at once, or for an unshared one either shares immediately at the level remembered in `GeneralSettingsModel.terminal.share_default` (`TerminalShareDefault::Ask` default, or `ReadOnly`/`ReadRun`; Settings → Terminal → Agent access → "When sharing a terminal") or opens a dialog (Read only / Read + run, with a "remember this" checkbox that writes the setting) when it's `Ask`. `TerminalAccess::ReadRun` only records the level today — running a command through it is AGE-584. `TerminalKind::Agent` registers pre-shared at `Read` and `set_access` leaves it alone, since it isn't the human's tab to unshare. `TerminalHandle::input_hidden()` (`crates/chatty-terminal/src/lib.rs`) reports a password prompt from termios alone: `ECHO` off while `ICANON` (canonical mode) is still on, which is what `sudo`/`ssh`/`su`/`passwd`/`getpass`/`read -s`/gpg `--pinentry-mode loopback` set — a line editor (readline/zle) or a full-screen program also turns `ECHO` off but leaves canonical mode too, so neither is mistaken for a password prompt; a full-screen passphrase box (pinentry-curses) and GUI pinentries look like any other raw-mode program to termios and are not caught (the passphrase itself is never on screen there), and Windows (no termios) always reads `None`. Every embedded read — the agent's own tab included — checks this first and fails with the fixed `terminal::HIDDEN_INPUT_MESSAGE` instead of the screen. A read flashes the source tab in the dock for `READ_FLASH` (1.5s), fed by `EmbeddedTerminals::subscribe_reads` (an unbounded channel per dock, sent to on every successful read, not a refused one).
