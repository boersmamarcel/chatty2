...
impl StreamManager {
    ...
    ///
    /// Text chunks are emitted immediately. GPUI's event loop coalesces `cx.notify()`
    /// calls within a frame, so no application-level batching is needed.
    /// All other chunk types are also forwarded immediately.
    pub fn handle_chunk(
        &mut self,
        conv_id: &str,
        chunk: StreamChunk,
