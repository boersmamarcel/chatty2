                    ...
                    match (models_result, providers_result, mcp_service_result, exec_settings_result, pending_approvals_result, pending_write_approvals_result) {
                        (Ok(models), Ok(providers), Ok(mcp_service), Ok(exec_settings), Ok(pending_approvals), Ok(pending_write_approvals)) => {
                            ...
                            if restored_count == 0 {
                                ...
                                if let Some(app) = weak.upgrade() {
                                    // Called directly (bypassing active_create_task guard) because
                                    // startup is sequential and the sidebar callback cannot fire yet.
                                    let task_result =
    ...
    pub fn create_new_conversation(
        ...
        if let Some(model_config) = model_config {
                ...
                .find(|p| p.provider_type == model_config.provider_type)
            {
                ...
                // Capture model list once (already read via `models` above)
                let models_list: Vec<(String, String)> = models
                    .models()
                    .iter()
                    .map(|m| (m.id.clone(), m.name.clone()))
                    .collect();
                ...
                cx.spawn(async move |_weak, cx| {
                    ...
                    // On failure, roll back the optimistic sidebar placeholder
                    if result.is_err() {
                        warn!("Conversation creation failed, removing sidebar placeholder");
                        sidebar
                            .update(cx, |sidebar, cx| {
                                let convs = cx
                                    .global::<ConversationsStore>()
                            ...
                            .map_err(|e| {
                                warn!(error = ?e, "Failed to roll back sidebar placeholder")
                            })
                            .ok();
                    }
