...
fn provider_type_to_string(pt: &ProviderType) -> String {
    match pt {
        ...
        ProviderType::HuggingFace => "Hugging Face".to_string(),
...
pub struct ModelsListView {
    ...
    preamble_input: Entity<InputState>,
...
impl ModelsListView {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let temperature_input = cx.new(|cx| InputState::new(window, cx).placeholder("1.0"));
    ...
    fn show_edit_model_dialog(
        ...
        // Create fresh input states pre-filled with model data
        let name_input = cx.new(|cx| {
            let mut state = InputState::new(window, cx).placeholder("e.g., GPT-4 Turbo");
            state.set_value(model.name.clone(), window, cx);
            state
        });
        let model_id_input = cx.new(|cx| {
            let mut state = InputState::new(window, cx).placeholder("e.g., gpt-4-turbo");
            state.set_value(model.model_identifier.clone(), window, cx);
            state
        });
        let temperature_input = cx.new(|cx| {
            let mut state = InputState::new(window, cx).placeholder("1.0");
            state.set_value(model.temperature.to_string(), window, cx);
            state
        });
        let preamble_input = cx.new(|cx| {
            let mut state =
                InputState::new(window, cx).placeholder("System instructions for the model");
            state.set_value(model.preamble.clone(), window, cx);
            state
        });
        let max_tokens_input = cx.new(|cx| {
            let mut state = InputState::new(window, cx).placeholder("e.g., 4096");
            state.set_value(
                model.max_tokens.map(|t| t.to_string()).unwrap_or_default(),
                window,
                cx,
            );
            state
        ...
        let top_p_input = cx.new(|cx| {
            ...
            state.set_value(
                model.top_p.map(|t| t.to_string()).unwrap_or_default(),
                window,
                cx,
            );
            state
        ...
        // Create provider select with current selection
        let providers = vec![
            "OpenAI".to_string(),
            "Anthropic".to_string(),
            "Gemini".to_string(),
        ...
        let provider_display = provider_type_to_string(&model.provider_type);
        let selected_index = providers
            .iter()
            .position(|p| p == &provider_display)
            .map(IndexPath::new);
        let provider_select = cx.new(|cx| SelectState::new(providers, selected_index, window, cx));
...
// Legacy function for backward compatibility with settings system
pub fn models_page() -> gpui_component::setting::SettingPage {
            ...
            SettingGroup::new()
                ...
                .items(vec![SettingItem::new(
                    ...
                    SettingField::render(|_options, window, cx| {
                        ...
                        let view =
                            if let Some(existing_view) = cx.try_global::<GlobalModelsListView>() {
